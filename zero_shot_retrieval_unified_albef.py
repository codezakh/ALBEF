...
from models.unified.albef_like import VisionLanguageLearner as ALBEF
...
def evaluation(model, data_loader, tokenizer, device, config):
    ...
    for image, img_id in tqdm(data_loader): 
        ...
        image_atts = torch.ones(image_patch_feat.size()[:-1],dtype=torch.long).to(image.device)
        image_feat = model.text_encoder.bert(
            inputs_embeds=image_patch_feat, 
...
def main(args, config):
    ...
    if args.checkpoint:    
        ...
        pos_embed_reshaped_m = interpolate_pos_embed(state_dict['visual_encoder_m.pos_embed'],model.visual_encoder)         
