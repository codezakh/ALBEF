"""
Pretrain a version of the single stream model with _only_ the image-text matching loss.
"""
import argparse
import os
...
def train(model, data_loader, optimizer, tokenizer, epoch, warmup_steps, device, scheduler, config, wandb_logger=None):
    ...
    for i, (image, text) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        loss_itm, loss_mlm = model(image, text_input, alpha = alpha)  
            
        loss = (loss_itm + loss_mlm) / 2
